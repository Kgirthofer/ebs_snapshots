...
describe 'delphic_snapshots::cron' do
  cached(:chef_run) do
  ...
  context "force run true" do 
    let(:chef_run) do
      default_chef_run do |node|
        node.set['delphic_snapshots']['back_up_schedule']['force_run'] = true
      end
    end
  ...
  context "when backup daily is selected" do
    let(:chef_run) do
      default_chef_run do |node|
        node.set['delphic_snapshots']['back_up_schedule']['daily'] = true
      end
    end
  ...
  context "when backup weekly is selected" do
    let(:chef_run) do
      default_chef_run do |node|
        node.set['delphic_snapshots']['back_up_schedule']['weekly'] = true
      end
    end
  ...
  context "when backup monthly is selected" do
    let(:chef_run) do
      default_chef_run do |node|
        node.set['delphic_snapshots']['back_up_schedule']['monthly'] = true
      end
    end
